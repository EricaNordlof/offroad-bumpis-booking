...
def booking_form():
    ...
    if request.method == "POST":
        ...
        estimated_price = total_price(child_count, adult_count, deliver, return_delivery, delivery_zone, tournament_kit, car_rental, cargo_bike)
        db = get_db()
        cur = db.execute(
            """
            INSERT INTO bookings (created_at, start_date, end_date, child_count, adult_count, name, email, phone, location, deliver, return_delivery, delivery_zone, tournament_kit, car_rental, cargo_bike, message, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'booked_unpaid')
            """,
            (datetime.now().isoformat(timespec="seconds"), start_date, end_date, child_count, adult_count, name, email, phone, location, deliver, return_delivery, delivery_zone, tournament_kit, car_rental, cargo_bike, message),
        )
