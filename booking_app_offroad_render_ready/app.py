...
# Aktiva rabattkoder. Koderna kontrolleras server-side så kunder kan inte ge sig själva rabatt
# genom att ändra i webbläsaren. Skrivs alltid med stora bokstäver i databasen och på fakturan.
COUPON_CODES = {
    "BLLACA09": 10,
    "YNGVE10": 10,
    "IGEN-1026": 10,
...
def total_price(
    ...
    subtotal = subtotal_price(
        child_count, adult_count, deliver, return_delivery, delivery_zone,
        tournament_kit, car_rental, cargo_bike
    )
...
def subtotal_price_for_booking(booking) -> int:
    return subtotal_price(
        ...
        int(booking["deliver"] or 0),
        int(booking["return_delivery"] or 0),
        booking["delivery_zone"] or "pickup",
        int(booking["tournament_kit"] or 0),
        int(booking["car_rental"] or 0),
        int(booking["cargo_bike"] or 0),
    )


def total_price_for_booking(booking) -> int:
    subtotal = subtotal_price_for_booking(booking)
    percent = int(booking["discount_percent"] or 0) if "discount_percent" in booking.keys() else coupon_discount_percent(booking["coupon_code"] if "coupon_code" in booking.keys() else None)
    return total_after_discount(subtotal, percent)


def booking_discount_amount(booking) -> int:
    subtotal = subtotal_price_for_booking(booking)
    percent = int(booking["discount_percent"] or 0) if "discount_percent" in booking.keys() else 0
...
def generate_payment_invoice_pdf(booking: sqlite3.Row, case_path: Path) -> str:
    ...
    subtotal = subtotal_price_for_booking(booking)
    discount_percent = int(booking["discount_percent"] or 0) if "discount_percent" in booking.keys() else 0
    discount_amount = discount_amount_for_subtotal(subtotal, discount_percent)
    total = total_price_for_booking(booking)
    coupon_code = booking["coupon_code"] if "coupon_code" in booking.keys() else ""
...
def generate_agreement_pdf(booking: sqlite3.Row, case_path: Path) -> str:
    ...
    lines = [
        ...
        f"Leveranszon: {DELIVERY_ZONES.get(booking['delivery_zone'] or 'pickup', DELIVERY_ZONES['pickup'])['label']}",
        f"Tillval: {selected_addons_text(booking)}",
        f"Rabattkod: {(booking['coupon_code'] if 'coupon_code' in booking.keys() and booking['coupon_code'] else '-')}",
        f"Rabatt: {int(booking['discount_percent'] or 0) if 'discount_percent' in booking.keys() else 0}%",
        f"Totalpris efter rabatt: {sek(total_price_for_booking(booking))}",
...
@app.get("/coupon-check")
def coupon_check():
    code = normalize_coupon_code(request.args.get("code", ""))
    percent = coupon_discount_percent(code)
    return jsonify({"valid": bool(percent), "code": code, "percent": percent})
